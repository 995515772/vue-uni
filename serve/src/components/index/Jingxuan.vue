<template>
	<div class="swiper">
		...
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>精选店铺</el-breadcrumb-item>
		</el-breadcrumb>
		<!-- comoom -->

		<el-button type="primary" style="margin-bottom: 20px;" @click="add">添加精选店铺图片</el-button>
		<el-dialog title="添加精选店铺图片" :visible.sync="addVisible" width="70%">
		...
		<el-dialog title="修改精选店铺图片" :visible.sync="setVisible" width="70%">
...
export default {
	...
	methods: {
		async getlist() {
			const { data: res } = await this.axios.get('http://127.0.0.1:9999/jingxuan');
			this.list = res.msg;
		},
		add() {
		...
		async set(id) {
			...
			const { data: res } = await this.axios.get(`http://127.0.0.1:9999/getjingxuan?id=${id}`);
		...
		addqueding() {
			this.$refs.addruleForm.validate(async value => {
				...
				const { data: res } = await this.axios.get(`http://127.0.0.1:9999/addjingxuan?imgUrl=${imgUrl}`);
				if (res.ok != 1) {
		...
		setqueding() {
			this.$refs.setruleForm.validate(async value => {
				...
				const imgUrl = img.replace(/&/g, '@');
				const { data: res } = await this.axios.get(`http://127.0.0.1:9999/setjingxuan?imgUrl=${imgUrl}&id=${id}`);
				if (res.ok != 1) {
		...
		async remove(id) {
			...
			const { data: res } = await this.axios.get(`http://127.0.0.1:9999/shanchujingxuan?id=${id}`);
			if (res.ok != 1) {
