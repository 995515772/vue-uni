<template>
	<div class="swiper">
		...
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>热门商品右</el-breadcrumb-item>
		</el-breadcrumb>
		<!-- comoom -->
...
export default {
	...
	methods: {
		async getlist() {
			const { data: res } = await this.axios.get('http://127.0.0.1:9999/shangpinright');
			this.list = res.msg;
		},
		add() {
		...
		async set(id) {
			...
			const { data: res } = await this.axios.get(`http://127.0.0.1:9999/getshangpinright?id=${id}`);
		...
		addqueding() {
			this.$refs.addruleForm.validate(async value => {
				...
				const { data: res } = await this.axios.get(`http://127.0.0.1:9999/addshangpinright?imgUrl=${imgUrl}&title=${title}`);
				if (res.ok != 1) {
		...
		setqueding() {
			this.$refs.setruleForm.validate(async value => {
				...
				const imgUrl = img.replace(/&/g, '@');
				const { data: res } = await this.axios.get(`http://127.0.0.1:9999/setshangpinright?imgUrl=${imgUrl}&id=${id}&title=${title}`);
				if (res.ok != 1) {
		...
		async remove(id) {
			...
			const { data: res } = await this.axios.get(`http://127.0.0.1:9999/shanchushangpinright?id=${id}`);
			if (res.ok != 1) {
